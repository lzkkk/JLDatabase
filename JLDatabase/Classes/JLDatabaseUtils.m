...
@implementation JLDatabaseUtils
...
+ (NSMutableDictionary*)checkTypeForValues:(NSMutableDictionary*)valuesDict
                                     class:(Class)clazz
{
    NSMutableDictionary *pdict = [[JLDatabaseUtils propertiesForClass:clazz] mutableCopy];
...
+ (NSString*)sqlCreateTable:(Class)clazz
    ...
    NSMutableDictionary* propertyDict = [[JLDatabaseUtils propertiesForClass:clazz] mutableCopy];
    ...
    if ([clazz conformsToProtocol:@protocol(JLDBManagerDelegate)] && [clazz respondsToSelector:@selector(columnPrimaryKey)]) {
    ...
    for (NSString *property in [propertyDict allKeys]) {
        ...
        NSString *coumnType = [JLDatabaseUtils columnTypeWithClass:clazz dict:propertyDict name:property];
        if (coumnType) {
            if (extra) {
...
+ (NSString *)sqlConditionClazz:(Class)clz
    ...
    if ([clz conformsToProtocol:@protocol(JLDBManagerDelegate)]) {
