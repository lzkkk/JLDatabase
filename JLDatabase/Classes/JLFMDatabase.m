...
@interface JLFMDatabase ()

@end

@implementation JLFMDatabase
...
- (BOOL)createTableWithTableName:(NSString *)tablename
    ...
    NSString *createSql = [JLDatabaseUtils sqlCreateTable:clz
                                                 tableName:tablename];
    __block BOOL ret = NO;
...
- (NSUInteger)insertTableParameFromArray:(NSArray *)array
    ...
    [_db inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (id obj in array) {
            ...
            NSMutableDictionary *param = [JLFMDatabaseUtils mutableDictionaryForObject:obj];
            param = [JLDatabaseUtils checkTypeForValues:param class:clz];
            ...
            if (update) {
                ...
                NSInteger count = 0;
                NSString *condition = [JLDatabaseUtils sqlConditionClazz:clz                                   param:param];
                if (!condition || ![condition length]) {
                    sqlQuery = [JLDatabaseUtils sqlInsertOrReplaceClazz:clz tableName:tablename param:param obj:obj];
                } else{
                    
                    condition = [NSMutableString stringWithString:[condition substringToIndex:[condition length]-4]];
                    NSString *sqlQueryCount = [JLDatabaseUtils sqlQueryCountClazz:clz tableName:tablename where:condition];
                    ...
                    if (count > 0) {
                        sqlQuery = [JLDatabaseUtils sqlUpdateClazz:clz tableName:tablename param:param where:condition];
                    } else{
                        sqlQuery = [JLDatabaseUtils sqlInsertClazz:clz tableName:tablename param:param obj:obj];
                    }
                }
...
- (BOOL)deleteTableParameFromClazz:(Class)clz
    ...
    __block BOOL ret = NO;
    NSString *sqlUpdate = [JLDatabaseUtils sqlDeleteClazz:clz tableName:tablename where:condition];
    if (!sqlUpdate) {
        ret = NO;
    }
...
- (BOOL)updateTableParameFromObj:(id)obj
    ...
    NSMutableDictionary *param = [JLFMDatabaseUtils mutableDictionaryForObject:obj];
    param = [JLDatabaseUtils checkTypeForValues:param class:[obj class]];
    NSMutableArray *arguments = [[NSMutableArray alloc] init];
    for (NSString *name in [param allKeys]) {
        [arguments addObject:(param[name]?:@"")];
    }
    __block BOOL ret = NO;
    NSString *sqlUpdate = [JLDatabaseUtils sqlUpdateClazz:[obj class]tableName:tablename param:param where:condition];
...
- (NSMutableArray *)querySql:(NSString *)sql
    ...
    [_db inDatabase:^(FMDatabase *db) {
        ...
        while ([set next]) {
            [array addObject:[JLFMDatabaseUtils objectWithClass:clz
                                                  fromResultSet:set]];
        }
        [set close];
    }];
...
- (NSArray *)queryTableFromClazz:(Class)clz
    ...
    NSString *sqlQuery = [JLDatabaseUtils sqlQueryClazz:clz
                                               tableName:tablename
                                                   where:condition];
...
- (NSInteger)queryCountTableFromClazz:(Class)clz
    ...
    NSString *sqlQuery = [JLDatabaseUtils sqlQueryCountClazz:clz tableName:tablename where:condition];
